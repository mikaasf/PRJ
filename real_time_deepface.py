                            ...
                            if values[ind] != "neutral":
                                if previous_annot:
                                    if previous_annot[0] != self.feelings_to_emotions.get(values[ind]):
                                        ...
                                        previous_annot = [self.feelings_to_emotions.get(values[ind]), self.timestamps_emotion[0], self.id_video, self.timestamps_emotion[len(self.timestamps_emotion) - 1]]
                                    else:
                                        previous_annot[3] = self.timestamps_emotion[len(self.timestamps_emotion) - 1]
                                else:
                                    previous_annot = [self.feelings_to_emotions.get(values[ind]),
                                                      self.timestamps_emotion[0], self.id_video,
                                                      self.timestamps_emotion[len(self.timestamps_emotion) - 1]]
                            self.buffer.clear()
